...
function Register_proc(bytes) {
    ...
    //pri
    Register_Msg.pri = ((((bytes[4] << 8) & 0xFF00) | (bytes[5] & 0xFF)) * 5) + "sec";
    //HB
    Register_Msg.hb = (bytes[6] * 30) + "sec";
    //Bluetooth receiving duration
    Register_Msg.BLEReceivingDuration = (bytes[7]) + "sec";
    //Network reconnection interval
    Register_Msg.Networkreconnectioninterval = (bytes[8] * 5) + "minutes";
...
//Message type: Device_Report_Rule   0x03
function Device_Report_Rule_proc(bytes) {
    ...
    for (var i = 0; i < (Device_Report_Rule_Msg.blockquantity); i++) {
        ...
        switch (bytes[3 + blocklength]) {
            ...
            case 0x03:
                Device_Report_Rule_Msg["MacBlock" + (i + 1) + "StartAddress"] = (Array(2).join(0) + (bytes[4 + blocklength] & 0xFF).toString(16).toUpperCase()).slice(-2);
                Device_Report_Rule_Msg["MacBlock" + (i + 1) + "EndAddress"] = (Array(2).join(0) + (bytes[5 + blocklength] & 0xFF).toString(16).toUpperCase()).slice(-2);
                blocklength = blocklength + 2 + 1;
                break;
        }
...
function Multi_Devices_Type_proc(bytes) {
    var Multi_Devices_Type_Uplink_Msg = {
        "type": 0,
        "number": 0,
    };

    //type
    Multi_Devices_Type_Uplink_Msg.type = "Multi-Device Type Message";
    //number
    Multi_Devices_Type_Uplink_Msg.number = (bytes[0] & 0x0F);

    var le = (bytes.length - 1) / Multi_Devices_Type_Uplink_Msg.number;
    for (var i = 0; i < (Multi_Devices_Type_Uplink_Msg.number); i++) {
        //beacon type ID
        ...
        //data数据
        var typedata = "";
        Multi_Devices_Type_Uplink_Msg["Device" + (i + 1) + "ID"] = devicetypeID;
        ...
        //data
        Multi_Devices_Type_Uplink_Msg["Device" + (i + 1) + "data"] = typedata;
        //rssi
        Multi_Devices_Type_Uplink_Msg["Device" + (i + 1) + "rssi"] = (bytes[le * (i + 1)].toString(10).toUpperCase() - 256) + "dBm";
    }
    return Multi_Devices_Type_Uplink_Msg;
}
...
function Device_Type_1_proc(bytes) {
    ...
    for (var i = 0; i < (Device_Type_1_Msg.number); i++) {
        ...
        for (var j = 0; j < le - 1; j++) {
            if (j == 0)
                type1data = (Array(2).join(0) + (bytes[1 + le*i + j].toString(16).toUpperCase())).slice(-2);
            else
                type1data = type1data + (Array(2).join(0) + (bytes[1 + le*i + j].toString(16).toUpperCase())).slice(-2);   
        }
...
function Device_Type_2_proc(bytes) {
    ...
    for (var i = 0; i < (Device_Type_2_Msg.number); i++) {
        ...
        for (var j = 0; j < le - 1; j++) {
            if (j == 0)
                type2data = (Array(2).join(0) + (bytes[1 + le*i + j].toString(16).toUpperCase())).slice(-2);
            else
                type2data = type2data + (Array(2).join(0) + (bytes[1 + le*i + j].toString(16).toUpperCase())).slice(-2);   
        }
...
function Device_Type_3_proc(bytes) {
    ...
    for (var i = 0; i < (Device_Type_3_Msg.number); i++) {
        ...
        for (var j = 0; j < le - 1; j++) {
            if (j == 0)
                type3data =  (Array(2).join(0) + (bytes[1 + le*i + j].toString(16).toUpperCase())).slice(-2);
            else
                type3data = type3data + (Array(2).join(0) + (bytes[1 + le*i + j].toString(16).toUpperCase())).slice(-2);   
        }
